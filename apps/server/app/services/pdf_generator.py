...
def generate_financial_report(
    ...
    """
    filename = (
        f"report_{user_name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    )
    ...
    story.append(
        Paragraph(
            f"<b>Gerado em:</b> {datetime.now().strftime('%d/%m/%Y %H:%M')}", styles["Normal"]
        )
    )
    ...
    if categories_summary:
        ...
        for cat in categories_summary:
            ...
            cat_data.append([cat["name"], f"R$ {cat['total']:,.2f}", f"{pct:.1f}%"])
