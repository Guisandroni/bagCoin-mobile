  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (Array.isArray(data))
        return { items: data, total: data.length } as GoalListResponse;
...
export function useCreateGoal() {
  ...
  return useMutation({
    mutationFn: (data: GoalCreate) => api.post<Goal>("/bagcoin/goals", data),
    onSuccess: () => {
...
export function useDeleteGoal() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: number) => api.delete(`/bagcoin/goals/${id}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["goals"] });
