"""Templates de resposta centralizados para o BagCoin.

Principios:
- Sem emojis excessivos
- Texto mais humanizado e direto
- Formatação limpa para WhatsApp
- Nomes amigáveis em vez de nomes técnicos
"""
...
def _fmt_date(raw) -> str:
    """Formata data/horário para formato brasileiro curto."""
...
def greeting(name: str = None, greeting_time: str = None) -> str:
...
def query_summary(summary: str) -> str:
    """Envolve resumo de consulta em wrapper limpo."""
    return summary
...
def budget_created(name: str, limit: float, period: str) -> str:
...
def goal_created(title: str, target: float, deadline: str = None) -> str:
...
def transaction_list(rows: List[Dict[str, Any]], title: str = "Transações") -> str:
    """Formata lista de transações de forma limpa."""
    if not rows:
        return "Nenhuma transação encontrada."
    lines = [f"{title}:"]
    for i, row in enumerate(rows, 1):
        # Extrai campos com nomes variados
...
def category_list(rows: List[Dict[str, Any]]) -> str:
    """Formata lista de gastos por categoria."""
    if not rows:
